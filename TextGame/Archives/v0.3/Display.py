...
def getInstruction(conn : Connection):
    ...
    try:
        if id == keys[0]:
            assert isinstance(args[0], int) and isinstance(args[1], int), 'Wrong data type for cmdDisplay'
            ID[id](args[0], args[1])
        if id == keys[1]:
            assert isinstance(args[0], Tuple), 'Wrong data type for change_size'
            ID[id](args[0])
        if id == keys[3]:
            log.info(type(data))
            assert isinstance(data, Frame), 'Wrong data type for FrameBuffer'
            ID[id](data)
        if id == keys[5]:
            data = ID[id]()
